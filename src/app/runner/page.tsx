  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="sticky top-0 z-50 bg-background/95 backdrop-blur-md border-b">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-bold text-sm">Runner Dashboard</h1>
              <div className="flex items-center gap-1.5">
                <div className={`w-2 h-2 rounded-full ${isSocketConnected ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                <span className="text-[10px] text-muted-foreground">
                  {isSocketConnected ? 'Live' : connectionError ? 'Reconnecting...' : 'Offline'}
                </span>
                {!isSocketConnected && (
                  <button onClick={retry} className="text-[10px] text-blue-500 hover:text-blue-600 font-medium ml-1">
                    Retry
                  </button>
                )}
                {isSimulated && isOnline && (
                  <Badge variant="secondary" className="text-[8px] h-3.5 px-1 ml-1">SIM GPS</Badge>
                )}
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {isOnline && (
              <Badge className="bg-emerald-500/10 text-emerald-600 border-emerald-500/30 rounded-full text-[10px]">
                <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full mr-1 animate-pulse" />
                Online
              </Badge>
            )}
            {incomingRequests.length > 0 && (
              <Badge className="bg-orange-500 text-white rounded-full text-[10px]">
      ...
      <div className={`relative transition-all duration-300 ${mapExpanded ? 'h-[50vh]' : 'h-[220px]'}`}>
        <RunnerDeliveryMap className="w-full h-full" onExpand={() => setMapExpanded(!mapExpanded)} />
      </div>
      ...
      <div className="flex-1 max-w-2xl mx-auto w-full px-4 py-4 pb-24 space-y-4">
        ...
        <div className="flex bg-muted rounded-xl p-1 gap-1">
          {VIEW_TABS.map(tab => {
            ...
            const showBadge = tab.id === 'active' && activeDelivery
            const showRequestBadge = tab.id === 'dashboard' && incomingRequests.length > 0
            return (
              <button
                key={tab.id}
              ...
              >
                ...
                {tab.label}
                {showBadge && <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-emerald-500 rounded-full" />}
                {showRequestBadge && <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-orange-500 rounded-full" />}
              </button>
            )
          })}
        </div>
              ...
              {isOnline && incomingRequests.length > 0 ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary" className="text-[10px]">{incomingRequests.length} available</Badge>
                  </div>
                  <RunnerIncomingRequest onAccept={acceptRequest} onCounter={counterOffer} onDecline={declineRequest} />
                </div>
              ) : isOnline ? (
                <div className="text-center py-12">
                  <Package className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
                  <p className="text-sm font-medium text-muted-foreground">Waiting for requests...</p>
                  <p className="text-xs text-muted-foreground">New delivery requests from nearby students will appear here</p>
                  <div className="flex items-center justify-center gap-2 mt-3">
                    <div className="w-2 h-2 bg-emerald-500 rounded-full animate-pulse" />
                    <span className="text-xs text-emerald-600">Listening for requests</span>
                  </div>
                </div>
              ) : (
                <div className="text-center py-12">
                  <MapPin className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
                  <p className="text-sm font-medium text-muted-foreground">Go online to start receiving requests</p>
                  <p className="text-xs text-muted-foreground">Toggle the switch above to start earning</p>
                </div>
              )}
            </motion.div>
          )}
            ...
            <motion.div key="active" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
              {activeDelivery ? (
                ...
                <div className="text-center py-16">
                  ...
                  <p className="text-xs text-muted-foreground mt-1">Accept a delivery request to get started</p>
