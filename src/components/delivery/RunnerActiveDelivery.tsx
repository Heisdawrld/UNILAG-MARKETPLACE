...
export default function RunnerActiveDelivery({ onStartNavigation, onConfirmPickup, onStartTransit, onConfirmDropoff, onCancel }: RunnerActiveDeliveryProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      {['runner_assigned', 'runner_en_route'].includes(activeDelivery.status) && !showCancelConfirm && <Button variant="ghost" className="w-full text-xs text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20" onClick={() => setShowCancelConfirm(true)}>Cancel Delivery</Button>}
