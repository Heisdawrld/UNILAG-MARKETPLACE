...
export function useRunnerGps({
  ...
  useEffect(() => {
    ...
    watchIdRef.current = navigator.geolocation.watchPosition(
      (position) => {
        ...
        streamLocation(latitude, longitude, heading, speed)
      },
  ...
  useEffect(() => {
    ...
    if (simulate) {
      locationIntervalRef.current = setInterval(() => {
        ...
        streamLocation(loc.lat, loc.lng, loc.heading, loc.speed)
      }, updateInterval)
    } else {
      locationIntervalRef.current = setInterval(() => {
        const { currentLat, currentLng, currentHeading, currentSpeed } = useRunnerStore.getState()
        if (currentLat && currentLng) streamLocation(currentLat, currentLng, currentHeading, currentSpeed)
      }, updateInterval)
    }
    return () => {
  ...
  useEffect(() => {
    ...
    heartbeatIntervalRef.current = setInterval(() => {
      if (socket?.connected) socket.emit('runner:heartbeat')
    }, heartbeatInterval)
    return () => {
  ...
  }, [enabled, socket, isSocketConnected, heartbeatInterval])
